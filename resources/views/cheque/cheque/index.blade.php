...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">{{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Cheque') }}</h3>
                    <!-- @can('add-'.str_slug('Cheque'))
                        <a class="btn btn-success pull-right" href="{{ url('/cheque/cheque/create') }}"><i
                                    class="icon-plus"></i> Add {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Cheque') }}</a>
                    @endcan -->
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @can('view-'.str_slug('Cheque'))
                                            <a href="{{ url('/cheque/cheque/' . $item->id) }}"
                                               title="View {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Cheque') }}">
                                        ...
                                        @can('edit-'.str_slug('Cheque'))
                                            <a href="{{ url('/cheque/cheque/' . $item->id . '/edit') }}"
                                               title="Edit {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Cheque') }}">
                                        ...
                                        @can('delete-'.str_slug('Cheque'))
                                            <form method="POST"
                                                  action="{{ url('/cheque/cheque' . '/' . $item->id) }}"
                                                  accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
                                                <button type="submit" class="btn btn_black btn-sm" title="Delete {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Cheque') }}" onclick="return confirm(&quot;Confirm delete?&quot;)">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <form id="cheque_form" method="POST" action="{{ url('/cheque/cheque') }}" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="modal-body">
                            <div class="form-group">
                                <label for="message-text" class="control-label">File:</label>
                                <input class="form-control" name="attachment" type="file" id="file" accept="application/pdf" value="{{ $cheque->attachment??''}}" >
