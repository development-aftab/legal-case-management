...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">{{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Category') }}</h3>
                    @can('add-'.str_slug('Category'))
                        <a class="btn btn-success pull-right" href="{{ url('/category/category/create') }}"><i
                                    class="icon-plus"></i> Add {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Category') }}</a>
                    @endcan
                                    ...
                                    <td>
                                        @can('view-'.str_slug('Category'))
                                            <a href="{{ url('/category/category/' . $item->id) }}"
                                               title="View {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Category') }}">
                                        ...
                                        @can('edit-'.str_slug('Category'))
                                            <a href="{{ url('/category/category/' . $item->id . '/edit') }}"
                                               title="Edit {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Category') }}">
                                        ...
                                        @can('delete-'.str_slug('Category'))
                                            <form method="POST"
                                                ...
                                                <button type="submit" class="btn btn-danger btn-sm"
                                                        title="Delete {{ preg_replace('/(?<=[a-z])[A-Z]|[A-Z](?=[a-z])/', ' $0', 'Category') }}"
